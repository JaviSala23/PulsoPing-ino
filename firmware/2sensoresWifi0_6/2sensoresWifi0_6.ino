  ...
  void setup() {
     ...
     if (strcmp(ssid, "default_ssid") != 0) {
        ...
        connectToWiFi();  // Si no es "default_ssid", trata de conectar
      } else {
      Serial.println("No se encontraron credenciales. Iniciando modo AP...");
      startAPMode();  // Si es "default_ssid", inicia el modo AP
      digitalWrite(LED_GREEN_PIN, LOW);  // LED verde apagado
      digitalWrite(LED_RED_PIN, HIGH);   // LED rojo encendido
    } 
  ...
  void readSensors() {
    ...
    if (!t1Failed) {
      ...
      sendData(json1);
       ...
       String json1 = "{\"temperature\":\"F\", \"placa\":1, \"puerto\":1}";
      sendData(json1);
    }
    ...
    if (!t2Failed) {
      ...
      sendData(json2);
       ...
       String json2 = "{\"temperature\":\"F\" , \"placa\":1, \"puerto\":2}";
      sendData(json2);
    }
  }
